...
# VISUAL 3) Consistency per position (CV_Rating)
...
# VISUAL 4) Age vs CV_Rating + tendency
plt.figure(figsize=(9, 6))
sns.regplot(
...
# VISUAL 5) Minutes vs CV_Rating + tendency
plt.figure(figsize=(9, 6))
sns.regplot(
...
# VISUAL 6) 3 Tables: Most inconsitency per rival difficulty 
# We need
# - CV_Rating_Facil, Rating_Facil_n
# - CV_Rating_Medio, Rating_Medio_n
# - CV_Rating_Dificil, Rating_Dificil_n
...
def make_table(cv_col, n_col, title, filename):
    ...
    # Filter por mínimo nº de partidos (con NaN -> 0 por seguridad)
    tmp = df.copy()
...
# 7) Classification REAL vs based on consistency
# (SIN PUNTOS) — usando el orden que has dado
# ============================================================
...
# 8) Specific metrics per position
...
# 2) MEDIOCAMPISTAS — Pass Accuracy
scatter_topN_metric_by_position(
    df,
    position="MF",
...
# VISUAL EXTRA — CV vs Rating_mean (Top 15)
TOP_N_HIGHLIGHT = 15
